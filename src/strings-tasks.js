...
function isPalindrome(str) {
  ...
  const palindrom = string.split('').reverse().join('');

  if (string.toLowerCase() === palindrom.toLowerCase()) {
    return true;
  }
...
function findLongestWord(sentence) {
  ...
  const numbersArr = arr.map((e) => e.length);
  const indexOfWorld = numbersArr.indexOf(Math.max(...numbersArr));

  return arr[indexOfWorld];
}
...
function encodeToRot13(str) {
  ...
  const result = str.replace(
    /[a-z]/gi,
    (symb) => falseAlphabet[realAlphabet.indexOf(symb)]
  );
